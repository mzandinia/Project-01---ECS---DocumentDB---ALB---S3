...
variable "task_requires_compatibilities" {
  description = "Requires compatibilities for the task definition"
  type        = list(string)
  default     = ["FARGATE"]
  validation {
    condition = alltrue([
      for compatibility in var.task_requires_compatibilities :
      contains(["FARGATE", "EC2"], compatibility)
    ])
    error_message = "Requires compatibilities must be either 'FARGATE' or 'EC2'."
  }
}
